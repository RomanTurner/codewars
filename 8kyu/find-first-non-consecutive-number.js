...
[] and [ x ] though? (This is an empty array and one with a
single number and is not tested for, but you can write your 
own example test. )
*/


firstNonConsecutive = (arr) => (
    (fNC = arr.find((element, index, array) => index && Math.max(element, array[index - 1]) - Math.min(element, array[index - 1]) > 1)),
     fNC === undefined ? null : fNC );
